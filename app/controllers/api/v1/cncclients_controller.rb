...
class CncclientsController < ApplicationController
  ...
  # GET /cncclients
  def index
    @cncclients = Tenant.find(params[:tenant_id]).cncclients
  ...
  def destroy
    @cncclient.destroy
    #@cncclient.update(isactive:0)
    render json: @cncclient.id
  end
  ...
  private
    ...
    def cncclient_params
      params.require(:cncclient).permit(:client_name, :email_id, :phone_number, :tenant_id,:company_name)
    end
end
end
end